...
parser = argparse.ArgumentParser(
    description="Takes an MBSolve problem \
...
parser.add_argument(
    "-s", "--save-path", help="Path to save output MP4.", required=False, default="./"
)
parser.add_argument(
    "-c",
    "--speed-of-light",
    help="Speed of Light in the system units.",
    default=0.1,
    required=False,
)
parser.add_argument(
    "-m", "--y-min", help="Minimum of the y-axis.", default=0.0, required=False
)
parser.add_argument(
    "-y", "--y-max", help="Maximum of the y-axis maximum", default=1.0, required=False
)
parser.add_argument(
    "--y-max-1",
    help="Maximum of the second y-axis maximum",
    default=1.0,
    required=False,
)
parser.add_argument(
    "-z",
    "--zoom",
    help="To use interpolation on the output \
    data, select the order of interpolation. (e.g. 2, 4). Note this may \
    introduce numerical artefacts.",
    default=1,
    required=False,
)
parser.add_argument("-p", "--fps", help="Frames per second", default=30, required=False)
parser.add_argument(
    "-a", "--atoms-alpha", help="Atoms alpha", default=0.2, required=False
)
parser.add_argument(
    "--c-line",
    action="store_true",
    help="Indicate the speed of light with vertical line",
    default=False,
)
parser.add_argument(
    "--peak-line",
    action="store_true",
    help="Indicate the pulse peak with vertical line",
    default=False,
)
...
field_fixed_frame_0 = fixed.rabi_freq(
    mb_solve_00, 0, speed_of_light, part="real", interp_kind="linear"
)
field_fixed_frame_1 = fixed.rabi_freq(
    mb_solve_00, 1, speed_of_light, part="real", interp_kind="linear"
)
...
if not show_c_line:  #  Hide speed of light indicator line
    c_line.set_visible(False)

(peak_line,) = ax.plot([], [], lw=2, color=pal[2])
peak_y = [y_min, y_max]

if not show_peak_line:  # Hide pulse peak indicator line
...
ax.set_ylabel(
    r"{0} Rabi Freq ($\Gamma / 2\pi $)".format(mb_solve_00.atom.fields[0].label),
    color=pal[2],
)
ax1.set_ylabel(
    r"{0} Rabi Freq ($\Gamma / 2\pi $)".format(mb_solve_00.atom.fields[1].label),
    color=pal[3],
)
...
def animate(i):
    ...
    ax.fill_between(
        x, 0.0, y_0, alpha=0.5, color=pal[2], clip_on=False, interpolate=True
    )
    ax1.fill_between(
        x, 0.0, y_1, alpha=0.2, color=pal[3], clip_on=False, interpolate=True
    )
    ...
    t_text.set_text(r"t = {:.1f} $/\Gamma$".format(tlist_fixed_frame[i]))
...
anim = animation.FuncAnimation(
    fig, animate, init_func=init, frames=len(tlist_fixed_frame), interval=25, blit=False
)
