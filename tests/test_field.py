...
class TestInit(unittest.TestCase):
    ...
    def test_init_default(self):
        ...
        self.assertEqual(
            field_00.rabi_freq_t_args, {"ampl_0": 1.0, "on_0": 0.0, "off_0": 1.0}
        )
        self.assertEqual(field_00.rabi_freq_t_func.__name__, "square_0")
        t_func = t_funcs.square(0)
        args = field_00.rabi_freq_t_args
        for t in [0.1, 0.3, 0.5, 0.7, 0.9]:
            self.assertEqual(field_00.rabi_freq_t_func(t, args), t_func(t, args))
    ...
    def test_incorrect_num_factors(self):
        ...
        with self.assertRaises(ValueError) as context:
            field.Field(coupled_levels=[[0, 1], [0, 2]], factors=[1.0])

        self.assertTrue(
            "The length of factors must be the same as the length "
    ...
    def test_from_json_str(self):
        ...
        self.assertEqual(
            self.field_02.rabi_freq_t_args, {"ampl_0": 1.0, "off_0": 0.7, "on_0": 0.3}
        )
        t_func = t_funcs.square(0)
        args = self.field_02.rabi_freq_t_args
        for t in [0.1, 0.3, 0.5, 0.7, 0.9]:
            self.assertEqual(self.field_02.rabi_freq_t_func(t, args), t_func(t, args))
...
class TestBuildRabiFreqTFunc(unittest.TestCase):
    ...
    def test_null(self):
        ...
        for t in [0.1, 0.3, 0.5, 0.7, 0.9]:
            self.assertEqual(
                self.field_00.rabi_freq_t_func(t, args), t_funcs.square(0)(t, args)
            )

        self.assertEqual(
            self.field_00.rabi_freq_t_args, {"ampl_0": 1.0, "on_0": 0.0, "off_0": 1.0}
        )

    def test_square(self):

        self.field_00.build_rabi_freq_t_func("square")
        self.field_00.build_rabi_freq_t_args({"on": 0.3, "off": 0.7, "ampl": 1.0})
    ...
    def test_ramp_onoff(self):
        ...
        self.field_00.build_rabi_freq_t_args(
            {"on": 0.3, "off": 0.7, "ampl": 1.0, "fwhm": 0.1}, 12
        )

        self.assertEqual(self.field_00.rabi_freq_t_func.__name__, "ramp_onoff_12")
        args = self.field_00.rabi_freq_t_args
        for t in [0.1, 0.3, 0.5, 0.7, 0.9]:
            self.assertEqual(
                self.field_00.rabi_freq_t_func(t, args), t_funcs.ramp_onoff(12)(t, args)
            )
