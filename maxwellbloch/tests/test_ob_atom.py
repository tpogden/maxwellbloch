...
class TestBuildInitialState(unittest.TestCase):
    ...
    def test_set_initial_state(self):
        oba = ob_atom.OBAtom(num_states=2, initial_state=[1.0, 0.0])
        self.assertEqual(oba.initial_state, oba.sigma(0, 0))
        oba = ob_atom.OBAtom(num_states=2, initial_state=[0.0, 1.0])
        self.assertEqual(oba.initial_state, oba.sigma(1, 1))

    def test_wrong_length(self):
        with self.assertRaises(ValueError):
            ob_atom.OBAtom(num_states=2, initial_state=[1.0])
        with self.assertRaises(ValueError):
            ob_atom.OBAtom(num_states=2, initial_state=[1.0, 0.0, 0.0])

    def test_trace_not_unity(self):
        with self.assertRaises(ValueError):
            ob_atom.OBAtom(num_states=2, initial_state=[2.0, 0.0])
        with self.assertRaises(ValueError):
            ob_atom.OBAtom(num_states=2, initial_state=[2.0, 2.0])
...
class TestBuildFields(unittest.TestCase):
    def test_one_field(self):
        ...
        self.assertDictEqual(
            oba.get_field_args(), {"ampl_0": 2.5, "on_0": 0.2, "off_0": 0.3}
        )
...
class TestJSON(unittest.TestCase):
    ...
    def test_to_from_json_str(self):
        ...
        self.assertEqual(ob_atom_00.to_json_str(), ob_atom_01.to_json_str())

    def test_to_from_json_str_02(self):

        ob_atom_03 = ob_atom.OBAtom().from_json_str(self.ob_atom_02.to_json_str())

        self.maxDiff = None
        self.assertEqual(self.ob_atom_02.to_json_str(), ob_atom_03.to_json_str())
    ...
    def test_from_json_file_02(self):
        ...
        ob_atom.OBAtom().from_json(json_path)
...
class TestBuildHDelta(unittest.TestCase):
    def test_two_level_no_detuning(self):
        ...
        np.testing.assert_array_almost_equal(oba.H_Delta.full(), H_Delta_test.full())
    ...
    def test_two_level_detuning(self):
        ...
        H_Delta_test = qu.Qobj([[0.0, 0.0], [0.0, -2 * np.pi * DETUNING]])
...
class TestBuildCOps(unittest.TestCase):
    def test_channels_factors_wrong_length(self):
        ...
        with self.assertRaises(ValueError):
            ob_atom.OBAtom.from_json_str(OBA_JSON)
