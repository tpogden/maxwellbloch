...
class TestSaveCSVRabiFreq(unittest.TestCase):
    ...
    def tearDown(self):
        for f in ['rabi_freq.csv', 'rabi_freq_abs.csv']:
            if os.path.exists(f):
                os.remove(f)
    ...
    def test_save_load(self):
        ...
        test_rf = np.loadtxt('rabi_freq.csv', delimiter=',', dtype=complex)
        test_rf_abs = np.loadtxt('rabi_freq_abs.csv', delimiter=',')

        self.assertTrue(np.allclose(test_rf, mb_solve_00.Omegas_zt[0]))
        self.assertTrue(np.allclose(test_rf_abs,
                                    np.abs(mb_solve_00.Omegas_zt[0])))
